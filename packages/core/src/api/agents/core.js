...
export async function updateAgentRegistrationCore(ctx, input) {
    ...
    try {
        ...
        if (Array.isArray(maybeRegistrations)) {
            ...
            const registryAddress = getChainContractAddress('AGENTIC_TRUST_IDENTITY_REGISTRY', parsed.chainId);
            if (registryAddress) {
...
export async function prepareAssociationRequestCore(ctx, input) {
    if (!input.did8004?.trim()) {
        throw new AgentApiError('did8004 parameter is required', 400);
    }
    const mode = input.mode ?? 'smartAccount';
