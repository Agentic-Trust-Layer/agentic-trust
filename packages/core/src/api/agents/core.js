...
export async function createAgentCore(ctx, input) {
...
export async function updateAgentRegistrationCore(ctx, input) {
    ...
    const uploadResult = await uploadRegistration(registrationObject);
    const client = await resolveClient(ctx);
...
export async function requestFeedbackAuthCore(ctx, input) {
...
export async function prepareFeedbackCore(ctx, input) {
    if (!input.did8004?.trim()) {
        throw new AgentApiError('did8004 parameter is required', 400);
    }
...
export async function prepareValidationRequestCore(ctx, input) {
    if (!input.did8004?.trim()) {
        throw new AgentApiError('did8004 parameter is required', 400);
    }
    const mode = input.mode ?? 'smartAccount';
    if (mode !== 'smartAccount') {
        throw new AgentApiError(`mode "${mode}" is not supported for validation requests. Only "smartAccount" mode is supported.`, 400);
    }
    const parsed = (() => {
        try {
            return parseDid8004(input.did8004);
    ...
    if (!validatorPrivateKey) {
        throw new AgentApiError('AGENTIC_TRUST_VALIDATOR_PRIVATE_KEY environment variable is not set', 500);
    }
    ...
    const { address: validatorAddress } = await createValidatorAccountAbstraction(validatorName, validatorPrivateKey, parsed.chainId);
    ...
    try {
        ...
        console.warn(`[prepareValidationRequestCore] Unable to check existing validation status for requestHash=${requestHash}`, error);
    }
...
export async function getFeedbackCore(ctx, input) {
    if (!input.did8004?.trim()) {
        throw new AgentApiError('did8004 parameter is required', 400);
    }
    const parsed = (() => {
...
export async function submitFeedbackDirectCore(ctx, input) {
    const client = await resolveClient(ctx);
