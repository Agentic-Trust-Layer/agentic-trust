 ...
 * 
 * To customize, replace getAgenticTrustClient with your app-specific client getter function
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const result = await handleCreateAgent(body, getAgenticTrustClient);
