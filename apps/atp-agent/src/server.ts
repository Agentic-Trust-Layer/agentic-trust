      ...
      try {
        ...
        if (subdomain) {
          try {
            ...
            const agentRecord =
              (await db
                .prepare('SELECT session_package FROM agents WHERE LOWER(agent_name) = LOWER(?)')
                .bind(agentName)
                .first<{ session_package: string | null }>()) ??
              (await db
                .prepare('SELECT session_package FROM agents WHERE LOWER(ens_name) = LOWER(?) OR LOWER(ens_name) = LOWER(?)')
