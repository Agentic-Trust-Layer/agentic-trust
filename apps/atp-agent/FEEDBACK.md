...
- **Authorization issuance**: `oasf:trust.feedback.authorization` (from the subject agent’s A2A endpoint) returns `feedbackAuth`
...
- **Auth issuance** (`oasf:trust.feedback.authorization`): subject agent issues `feedbackAuth` to the reviewer
