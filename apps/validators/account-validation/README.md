...
A simple Express service that processes account validation requests for agents. The validator reads validation requests from the validation registry contract, validates that agent accounts exist and are correctly configured, and submits validation responses with a score of 100 for valid agents.
...
- Validates agent accounts exist and are correctly configured
...
2. **Validator Address**: The validator address (AA address) is derived from `AGENTIC_TRUST_VALIDATOR_PRIVATE_KEY` using the same method as in the admin app (seed: 'account-validation'). This is the address that validation requests are sent to.
...
4. **Validation**: For each request:
   ...
   - Validates the agent's account exists and is correctly configured
   - Verifies the account address matches the expected agent account
   - Checks that the account is properly set up
...
- The validator address (AA address) uses the same account abstraction address calculation as the admin app (seed: 'account-validation')
