...
2. **Validator Address**: The validator address (AA address) is derived from `AGENTIC_TRUST_VALIDATOR_PRIVATE_KEY` using the same method as in the admin app (seed: 'account-validator'). This is the address that validation requests are sent to.
...
- The validator address (AA address) uses the same account abstraction address calculation as the admin app (seed: 'account-validator')
