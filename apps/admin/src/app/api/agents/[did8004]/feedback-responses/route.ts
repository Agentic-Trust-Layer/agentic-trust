...
export async function GET(
  ...
  try {
    ...
    const result = await client.searchFeedbackAdvanced({
    ...
    console.error('[API] Error fetching feedback responses:', error);
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Failed to fetch feedback responses',
      },
      { status: 500 },
    );
  }
