...
export default function LineaEnsAgentViewPage() {
  const auth = useAuth();
  ...
  const lookup = useCallback(async () => {
    ...
    try {
      const resp = await fetch(
        `/api/ens/linea/agent?chainId=${encodeURIComponent(String(chainId))}&name=${encodeURIComponent(n)}&org=${encodeURIComponent(org)}`,
      );
  ...
  return (
    <>
      ...
      <main style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
        <h1 style={{ margin: 0, fontSize: '1.6rem' }}>ENS Agent View</h1>
        <p style={{ marginTop: '0.5rem', color: palette.textSecondary }}>
          Looks up agent info directly from the ENS registry/resolver on the selected chain.
        </p>

        <div style={{ marginTop: '1.25rem', border: `1px solid ${palette.border}`, borderRadius: 12, padding: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <label style={{ fontWeight: 700, color: palette.textSecondary }}>Agent name</label>
            <input
              value={name}
          ...
          <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            {fullNamePreview ? (
              <span style={{ fontFamily: 'monospace', color: palette.textSecondary }}>
                {fullNamePreview}
              </span>
            ) : null}
          </div>
          ...
          <div style={{ marginTop: '1.25rem', border: `1px solid ${palette.border}`, borderRadius: 12, padding: '1rem' }}>
            ...
            {listError ? (
              ...
              <div style={{ marginTop: '0.75rem', display: 'grid', gridTemplateColumns: '1fr', gap: '0.35rem' }}>
                ...
                {labels.length > 500 ? (
                  <div style={{ marginTop: '0.5rem', color: palette.textSecondary }}>
                    Showing first 500.
                  </div>
                ) : null}
              </div>
        ...
        {result && (
          <div style={{ marginTop: '1.25rem', border: `1px solid ${palette.border}`, borderRadius: 12, padding: '1rem' }}>
            {'ok' in result && result.ok ? (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: '0.5rem 0.75rem' }}>
                  <div style={{ color: palette.textSecondary }}>ENS name</div>
                  <div style={{ fontFamily: 'monospace' }}>{result.ensName}</div>

                  <div style={{ color: palette.textSecondary }}>Account</div>
                  <div style={{ fontFamily: 'monospace' }}>{result.account ?? '—'}</div>

                  <div style={{ color: palette.textSecondary }}>URL</div>
                  <div style={{ fontFamily: 'monospace', wordBreak: 'break-all' }}>{result.agentUrl ?? '—'}</div>

                  <div style={{ color: palette.textSecondary }}>Image</div>
                  <div style={{ fontFamily: 'monospace', wordBreak: 'break-all' }}>{result.image ?? '—'}</div>

                  <div style={{ color: palette.textSecondary }}>Description</div>
                  <div style={{ whiteSpace: 'pre-wrap' }}>{result.description ?? '—'}</div>
                </div>
              </>
            ) : (
              <div style={{ color: palette.dangerText, fontWeight: 700 }}>
                {result.error}
              </div>
            )}
          </div>
