/**
 * AgenticTrust Client setup for admin app (server-side)
 *
...
import { getChainRpcUrl, DEFAULT_CHAIN_ID } from '@agentic-trust/core';
...
export async function getAdminClient(): Promise<AgenticTrustClient> {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        'Could not access cookies, using environment variable for private key',
      );
    }
